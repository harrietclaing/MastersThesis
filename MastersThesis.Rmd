...
_(approx. 800, CURRENTLY 646)_

There have been many international environmental agreements (IEAs) entered into between countries to reduce environmental degradation which have fallen short of their objectives. For the non-renewable resources within the economy to be allocated efficiently in a way that considers sustainability in the future generations requires co-operation among countries (Buchholz _et al._, 2005). In practice, true co-operation that is ensured by credible commitment to policy preferences is difficult to achieve, and in fact, international agreements between countries may be counterproductive if citizens engage in strategic voting. 

In this thesis, the framework from Buchholz _et al._ (2005) is employed and extended for studying the issue of international co-operation regarding environmental and sustainable policies (?). _summary of the Buchholz paper, incl symmetry assumed between the countries and consumption assumptions in the model. Include the consumption reduction in exchange for side payments/compensation in return (check)_

_I extend this framework in two novel ways. First, I present a novel simplified framework that more directly represents sustainability in a formal: If consumption is not sustainable, then there is a potential for a crisis where consumption options are severely reduced from past experience. This is a contribution as it emphasizes the fundamental non-linearity inherent in a potential climate disaster. Second, I use a numerical approach that allows the study of more nuanced questions, specifically asymmetries between countries which are not feasible in a purely analytical model."_

_Then you can follow up with the disclaimer: Replicating the full analysis of Buchholz using this new framework is beyond the scope of this Master's thesis, but I present the following results: I compute the outcomes under perfect cooperation (which I model as a global democracy), and under a fully non-cooperative game where the global population is divided into two countries, each electing a local representative taking the consumption decisions of the other country as given. _
...
_Introduction to organise Lit Review: "First, resource extraction studies in a single country context. Then information aggregation in a single economy, then coordination on these issues across countries (basically only Buchholz?). This build up nicely to conclude with a paragraph that says you focus on the interaction of these things by presenting a much simplified model of the extraction literature's main ideas so that it can be incorporated in a political economy model with cross country coordination_
...
To introduce a role for political aggregation of private information in the model, we assume that there are different beliefs about the severity of the _climate crisis_ if the _tipping point_ is crossed. Specifically, we assume as in Besley & Coate (1998)?? that individuals differ in their beliefs about the level of consumption that would be possible if the crisis occurs ($\underline{c}$). These beliefs are determined by private signals $\hat{\underline{c}}_k$ which consumers receive.
...
_(approx 758...aim for 1200?)_

_"The key concern in this paper, as in Buchholz..., is the failure of coordination of aggreements across countries. I model this in a similar way, but only consider the non-cooperative outcomes in this thesis. I assume that there are two countries (a partition of the global populations) that each functions as an independent democracy. The populations in each country elect their leaders independently, and the leader implements their preferred choice of consumption, taking as given the consumption choice of the other country. The key mechanisms is that each country independently selects their own consumption value, but that the risk of a global crisis is determined by the average level of consumption in the global economy. Thus, consumers in a given country do not fully internalize the costs and risks of their choices in the global setting._  

## Economy {-}

To develop the model set-up further, the impact of another country's consumption level on the optimal level of consumption is incorporated. Accordingly, this model specification introduces two countries; country A and country B. The implications of a two-country model which considers the interdependence between countries is more realistic, as the global setting is increasingly globalised in terms of greater international trade of goods and services (Ortiz-Ospina, 2018). A prime example is the consideration of the impact of an IEA, as considered in Buchholz _et al._ (2005). _This is especially worth investigation considering that it has been found that achieving the Paris agreement's limit of global warming to two degrees Celcius cannot be achieved by one country acting unilaterally (Ferrari & Pagliani, 2021)._ 

## Consumers {-}

As in the global democracy model, each individual in each country receives private signals $\hat{\underline{c}}_k$ which determine their optimal level of individual consumption and this maps onto their optimal probability of crisis. Individuals which are signalled to believe that the climate crisis would not reduce $c_2$ by much, then a higher probability of this climate crisis occurring is not worrisome. These private signals are drawn again from a distribution between $c_L$ and $c_H$, and are identically distributed in each country unless, unless adjustable parameters are altered to create asymmetries. We study two dimensions of asymmetries in this model specification, namely in the relative size and bias in the beliefs of the countries.

Individuals in the global economy are split into two different country populations, after each individual has received their private signals, according to a stochastic assignment. Each individual is divided probabilistically into either country A or country B to ensure random splitting. 
...
To compare the voting outcomes between different scenarios, three variations of the two-country model are estimated. Firstly, the 0.1 country A share case in which the size of two-countries differ dramatically. Secondly, the 50/50 country split case in which each country is equal in size. Lastly, the 0.9 bias for country A case in which country A is more likely to contain individuals who received a higher private signal, and thus have a higher optimal consumption level and higher optimal probability of crisis.

## Voters {-}

As in the global democracy, the expected utilities for every possible candidate are computed under the assumption that each citizen runs for election and promises to implement their ideal consumption if elected. Accordingly, each voter considers any pairwise competition based on their expected utility and their own belief about $\tilde{c}$ as formed due to their private signal $\hat{\underline{c}}_k$ if any candidate implements their promised, preferred policy.
...
In the case of strategic considerations that are not fully internalised, there are two main findings. Non-cooperative results between two parties are found to be worse than cooperative results, and Nash bargaining results are found to lie somewhere in between these two polarised results. Thus the focus of this paper is to analyze how the non-cooperative results depend on asymmetries which Buchholz et al do not study.  leaving the bargaining results for future research. 

# Discussion and results 

_(1130)_

To analyse whether THE LACK OF co-operation in the political economy between two countries is harmful for environmental policies and mitigating climate crisis, the global democracy model set-up is compared to three variations of the two-country case. Buchholz _et al._ (2005) compares the isolationist case of one country implementing climate policies unilaterally against the bargaining case with an IEA implemented between two countries to analyse the impact of IEAs on environmental policy. Similarly, this paper compares the global democracy of one voter population with a uniform distribution of beliefs against the Nash equilibrium solution found in the case of two countries which bargain together with three variations; namely the asymmetric country size model, the symmetric size model and the asymmetric bias size model.

I would first give a clean analysis of the symmetric case
- give the country sizes (population) explicitly, the kernel densities showing that the distribution of optimal choices are identical and the best response function figures (discuss the values in the text as well)

Then assymetric size (same sequence)
- 

Then asymmetric opinions (bias) (same sequence)

Then summarize in the table. Add an additional column for the global outcomes, and add country sizes and median desires as rows. 	
